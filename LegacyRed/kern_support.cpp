...
bool Support::processKext(KernelPatcher &patcher, size_t index, mach_vm_address_t address, size_t size) {
    if (kextRadeonSupport.loadIndex == index) {
        ...
        RouteRequestPlus requests[] = {
            ...
            {"__ZN14AtiBiosParser126translateAtomConnectorInfoERN30AtiObjectInfoTableInterface_V117AtomConnectorInfoER13ConnectorInfo", wrapTranslateAtomConnectorInfo, orgTranslateAtomConnectorInfo},
...
void Support::autocorrectConnector(uint8_t connector, uint8_t sense, uint8_t txmit, uint8_t enc, Connector *connectors, uint8_t sz) {
...
void Support::updateConnectorsInfo(void *atomutils, t_getAtomObjectTableForType gettable, IOService *ctrl, Connector *connectors, uint8_t *sz) {
	if (atomutils) {
	...
	if (cons) {
		...
		if (atomutils) {
			...
			if (displayPathNum == connectorObjectNum)
				...
				DBGLOG("support", "getConnectorsInfo found different displaypaths %u and connectors %u", displayPathNum, connectorObjectNum);
		}
...
void Support::autocorrectConnectors(uint8_t *baseAddr, AtomDisplayObjectPath *displayPaths, uint8_t displayPathNum, AtomConnectorObject *connectorObjects,
	...
	for (uint8_t i = 0; i < displayPathNum; i++) {
		...
		DBGLOG("support", "autocorrectConnectors found txmit %02X enc %02X sense %02X for %u connector", txmit, enc, sense, i);
	}
}


uint32_t Support::wrapGetConnectorsInfo(void *that, Connector *connectors, uint8_t *sz) {
    auto props = callback->currentPropProvider.get();
    callback->updateConnectorsInfo(nullptr, nullptr, *props, connectors, sz);
	uint32_t code = FunctionCast(wrapGetConnectorsInfo, callback->orgGetConnectorsInfo)(that, connectors, sz);
...
uint32_t Support::wrapTranslateAtomConnectorInfo(void *that, AtomConnectorInfo *info, Connector *connector) {
	uint32_t code = FunctionCast(wrapTranslateAtomConnectorInfo, callback->orgTranslateAtomConnectorInfo)(that, info, connector);
			...
			for (uint8_t i = 0; i < ucNumberOfSrc; i++) {
				...
				if (((usSrcObjectID & OBJECT_TYPE_MASK) >> OBJECT_TYPE_SHIFT) == GRAPH_OBJECT_TYPE_ENCODER) {
					...
					if (getTxEnc(usSrcObjectID, txmit, enc))
						callback->autocorrectConnector(getConnectorID(info->usConnObjectId), getSenseID(info->i2cRecord), txmit, enc, connector, 1);
					break;
				}
			}
